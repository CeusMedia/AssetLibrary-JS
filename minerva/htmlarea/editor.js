...
this.imgURL = _images_url;
this.minURL = _minerva_url;
//if (_custom1) this.custom1 = _custom1;
this.debug  = 0;
...
this.btnList = {
    ...
    "custom1":           ['custom1',         'Save changes', 		 'editor_action(this.id)',  'ed_link.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  var tblOpen  = '<table border=0 cellspacing=0 cellpadding=0 class="cM" unselectable="on"><tr><td style="border: none; padding: 1 0 0 0"><nobr>';
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      // fontname
      if (btnName == "fontname") {
        toolbar += '<select class="editor" id="_' +objname+ '_FontName" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; font-size: 8pt;">';
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '<select class="editor" id="_' +objname+ '_FontSize" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 8pt;">';
      ...
      if (btnName == "fontstyle") {
        toolbar += '<select class="editor" id="_' +objname+ '_FontStyle" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 8pt;">';
  ...
  var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 bgcolor="" class="editor" width=' + config.width + ' unselectable="on"><tr><td>\n'
  + toolbar
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "popups/insert_image.php", editdoc, "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
  ...
  var TextEdit   = '<textarea class="editor" ID="_' +objname + '_editor" style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px;"></textarea>';
  var RichEdit   = '<iframe class="editor" ID="_' +objname+ '_editor"    style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ ';"></iframe>';
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
    ...
    if (config.stylesheet) {
      ...
      html += '<link href="' +config.imgURL+ 'editor.css" rel="stylesheet" type="text/css">\n';
    html += '<style>\n';
    for (var i in config.fontstyles) {
      var fontstyle = config.fontstyles[i];
    ...
    html += '</style>\n'
      + '</head>\n'
      + '<body class="editor" contenteditable="true" topmargin=1 leftmargin=1'
...
function editor_cMenu_generate(editorWin,objname) {
  ...
  var cMenuHeader = ''
    + '<div class="cContext" id="_'+objname+'_cMenu" onblur="editor_cMenu(this);" oncontextmenu="return false;" onselectstart="return false">'
    + '<table class="cContext" border=0 cellspacing=0 cellpadding=0 width="100%">'
