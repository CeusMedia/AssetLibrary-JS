...
//			zoomY = Math.min(Math.max(zoomY, 0), image.data('zoomMaxY'));							//  avoid lense to leave zoomed image on y axis
		...
		function setupImage(image){
			...
			$(imageZoom).bind('load ready', {image: image}, function(event){						//  when zoomed image is loaded
				...
				image.data('zoomWidth', zoom.attr('width'));										//  store width of zoomed image
				image.data('zoomHeight', zoom.attr('height'));										//  store height of zoomed image
				...
				zoom.parent().css('background-image', 'url('+zoom.attr('src')+')');					//  copy zoomed image into lense background
